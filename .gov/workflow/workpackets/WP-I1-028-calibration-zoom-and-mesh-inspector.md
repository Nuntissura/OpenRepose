...
- 2026-05-03: Operator authorized fast-track. Scope reduced back to M for this round to fit the session: shipped (1) zoom + pan via QGraphicsView (`_ZoomableImageView` replaces `_ClickablePortrait`; mouse-wheel zoom anchored under cursor; middle-mouse drag pans; click-with-no-movement places marker at image-space coord); (3) always-on MediaPipe-detected overlay in `render_calibration_overlay` (`detected_positions={name: (x, y)}` kwarg drawn as dim dots before any operator marker exists); plus a `Reset zoom` button. Sizing constraint preserved: DOCK_WIDTH_CAP=320 sizeHint so the dock width stays bounded. Detected positions sourced from rig.raw_face_mesh + MEDIAPIPE_FACEMESH_INDEX_BY_MARKER. **Deferred to follow-up WP-I1-034**: frontal mesh inspector with angle slider, drag-to-move operator markers, right-click-delete, new `delete_markers` LLM command. 5 new tests cover the always-on overlay + zoom view + reset button + detected_positions helper. Full suite 328/328. Audit clean. Status IN-PROGRESS -> REVIEW.
- 2026-05-03: Operator GUI inspection — middle-mouse pan didn't work for them and isn't preferred. They want Photoshop-convention spacebar+left-click-hold for pan. Status REVIEW -> IN-PROGRESS for the rebinding fix. Removed middle-mouse pan handling; added keyPressEvent / keyReleaseEvent on `_ZoomableImageView` that switch dragMode to ScrollHandDrag while spacebar is held + show open-hand cursor. Mouse press/release skip click-emit when space is held so panning doesn't accidentally place a marker. setFocusPolicy(StrongFocus) so the view receives key events. New regression test `test_calibration_view_pan_via_spacebar_left_click` asserts the dragMode toggles correctly on space press/release. Full suite 329/329. Audit clean. Status IN-PROGRESS -> REVIEW.
